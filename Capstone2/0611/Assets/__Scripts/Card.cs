...
public class Card : MonoBehaviour
{
	...
	private void OnMouseDown()
	{
		...
		// 보물(트레저) 스테이지에 진입했을때
		else if (GameManager.instance.cardManager.isTreasure)
        {
			CardManager.instance.AddCardToDeckInfo(cardInfo);
			CardManager.instance.isTreasure = false;
			CardManager.instance.ClearTreasure();
			GameObject.Find("Treasure Panel").SetActive(false);
